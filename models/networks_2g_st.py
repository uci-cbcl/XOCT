...
class VGGPerceptualLoss(nn.Module):
    """
    A module that uses a subset of pretrained VGG19 features to compute a perceptual loss.
    """
    def __init__(self, layers=["relu2_2", "relu3_3"], requires_grad=False):
        super().__init__()
        
        # Load the pretrained VGG19
        vgg_pretrained = t_models.vgg19(pretrained=True).features
        
        # We will selectively keep only part of the layers
        self.vgg_layers = nn.ModuleDict()
        
        # Map layer indices to layer names for vgg19.features
        # Example layers:
        #   0 = conv1_1, 1 = relu1_1, 2 = conv1_2, 3 = relu1_2, 4 = pool1
        #   5 = conv2_1, 6 = relu2_1, ...
        # We want to capture features at certain relu activations.
        ...
        # Building a temporary sequential model up to each target layer name
        name_to_layer_index = {v: int(k) for k, v in layer_name_mapping.items()}
        # e.g. name_to_layer_index["relu2_2"] = 8
        
        max_index = max(name_to_layer_index[n] for n in layers)
        
        # We only need up to the largest layer index among the layers we want
        truncated_vgg = vgg_pretrained[: max_index + 1].eval()
        
        # Freeze the parameters if requires_grad=False
        if not requires_grad:
            for param in truncated_vgg.parameters():
    ...
    def forward(self, x):
        """
        Returns a dictionary of {layer_name: feature_map}, 
        where layer_name is e.g. "relu2_2" if it's in self.selected_layers.
        """
        out_features = {}
        

        # We need a mapping from layer index to layer name
        index_to_name = {
            1: "relu1_1", 3: "relu1_2", 6: "relu2_1", 8: "relu2_2",
        ...
        for i, layer in enumerate(self.truncated_vgg):
            x = layer(x)
            if i in index_to_name:
                layer_name = index_to_name[i]
                if layer_name in self.selected_layers:
                    out_features[layer_name] = x
        
        return out_features

    def perceptual_loss(self, generated, target):
        """
        Compute an L1 perceptual loss by comparing features of generated and target
        images at certain VGG layers.
        """
        # Both images should be normalized in the same way VGG was trained
        ...
        gen_feats = self(generated)
        tgt_feats = self(target)
        
        loss = 0.0
        for l in self.selected_layers:
            loss += F.l1_loss(gen_feats[l], tgt_feats[l])
